...
export default function DashboardPage() {
  const notes = [
    {
      ...
      summary: "Block-based editor roadmap...",
    ...
    },
    {
      ...
      summary: "User acquisition funnels...",
  ...
  const allNotes = [
    { id: "1", title: "Feature: GitHub Sync Integration", subtitle: "Last edited by you • 2 hours ago", icon: "📝", tags: ["G-Drive", "Notion", "GitHub"] },
    { id: "2", title: "Home Renovation Ideas", subtitle: "Shared with Sarah • Yesterday", icon: "🏠", tags: ["Private"] },
    { id: "3", title: "Content Calendar - Q4", subtitle: "Draft • Oct 12, 2023", icon: "🏷️", tags: [] },
  ];
    ...
    <div className="flex-1 flex flex-col relative w-full h-full text-[#F5F7FA]">
      <section className="h-40 relative overflow-hidden shrink-0">
        ...
        <div className="absolute bottom-8 left-10">
          <h1 className="text-3xl font-bold tracking-tight text-white">Welcome back, Chemitha</h1>
          <p className="text-[#9CA3AF] text-sm mt-1">You have 12 notes and 3 pending syncs.</p>
        </div>
      </section>
      ...
      <div className="px-10 mt-8 shrink-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {notes.map((note) => (
            <Link href={`/dashboard/notes`} key={note.id} className="block">
              <div className="bg-[#181A20] border border-[#2A2E37] p-4 rounded-xl hover:border-[#7C5CFF66] transition-colors cursor-pointer group">
                ...
                <p className="font-medium text-sm truncate group-hover:text-[#F5F7FA]">{note.title}</p>
                <p className="text-xs text-[#4B5563] mt-1 truncate">{note.summary}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <div className="px-10 mt-10 shrink-0 mb-20 overflow-visible">
        <h3 className="text-xs uppercase font-bold tracking-widest text-[#4B5563] mb-4">All Notes</h3>
        <div className="space-y-2">
          {allNotes.map((note) => (
            <div key={note.id} className="group bg-[#181A20] border border-[#2A2E37] p-4 rounded-xl flex items-center gap-4 hover:border-[#7C5CFF] transition-all cursor-pointer">
              <div className="text-xl shrink-0">{note.icon}</div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate group-hover:text-white transition-colors">{note.title}</p>
                <p className="text-xs text-[#4B5563] truncate">{note.subtitle}</p>
              </div>
              
              {note.tags.length > 0 && (
                <div className={`flex items-center gap-2 ${note.tags.includes("Private") ? "" : "opacity-0 group-hover:opacity-100 transition-opacity"}`}>
                    {note.tags.includes("Private") ? (
                        <div className="flex gap-1 items-center">
                            <div className="w-1.5 h-1.5 rounded-full bg-green-500"></div>
                            <span className="text-[10px] text-[#4B5563]">Private</span>
                        </div>
                    ) : (
      ...
      <div className="fixed bottom-8 right-8 z-50">
        <button className="w-14 h-14 bg-[#7C5CFF] rounded-full flex items-center justify-center text-white shadow-2xl shadow-[#7C5CFF66] hover:scale-105 transition-transform" aria-label="New Note">
